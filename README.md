...
## 前言
        ...
        但是这个设计还能发挥余热，参与我接下来的电子系统设计之中去。本设计选择Vivado 2018.3平台，采用Verilog HDL语言开发，开发板设备为Zynq-7020，包含FPGA系统（含DAC模块）、麦克风输入信号放大电路以及网页版上位机，FPGA调用了PL端进行数字信号处理的实现（FFT输出以及功率谱的计算），PS端则进行输出结果的判断和处理。本次电子系统设计的成果得分为A+（100分，含设计分95分以及问答附加分5分）。
